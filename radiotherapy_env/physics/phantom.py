...
def _make_irregular_mask(
    grid_size: int, cx: float, cy: float, r: float,
    rng: np.random.Generator, n_modes: int = 5, irregularity: float = 0.28,
) -> np.ndarray:
    """
    Create an irregular blob mask using Fourier radial perturbations.
    The boundary radius varies per angle: r(θ) = r + Σ aₖ·cos(k·θ + φₖ)
    This produces a smooth but non-circular shape that looks like a real tumor.
    """
    y_g, x_g = np.mgrid[:grid_size, :grid_size]
    dx = x_g.astype(np.float32) - cx
    dy = y_g.astype(np.float32) - cy
    angles = np.arctan2(dy, dx)          # per-pixel angle from center
    dist   = np.sqrt(dx ** 2 + dy ** 2)  # per-pixel distance from center

    # Sum of Fourier modes — k=2 gives big lobes, higher k gives finer bumps
    r_boundary = np.full_like(angles, r, dtype=np.float32)
    for k in range(2, n_modes + 2):
        amp   = float(rng.uniform(0.0, irregularity * r / k))
        phase = float(rng.uniform(0.0, 2 * np.pi))
        r_boundary += amp * np.cos(k * angles + phase)
...
class PediatricBrainPatientGenerator:
    ...
    def generate(self, rng: np.random.Generator) -> PatientPhantom:
        ...
        # Tumor: adjacent to brainstem (2-3 voxels gap from brainstem surface)
        offset_angle = rng.uniform(0, 360)
        angle_rad_dir = np.deg2rad(offset_angle)
        cos_d, sin_d = np.cos(angle_rad_dir), np.sin(angle_rad_dir)

        # Actual brainstem ellipse boundary radius in this direction
        bs_r_at_angle = (bs_rx * bs_ry) / (
            np.sqrt((bs_ry * cos_d) ** 2 + (bs_rx * sin_d) ** 2) + 1e-8
        )
        t_r = rng.uniform(3, 5)                          # base radius
        gap = rng.uniform(1.5, 3.0)                     # voxel gap: tumor surface → brainstem surface
        offset_r = bs_r_at_angle + t_r + gap            # guaranteed no overlap

        t_cx = bs_cx + offset_r * cos_d
        t_cy = bs_cy + offset_r * sin_d
        tumor_mask = _make_irregular_mask(G, t_cx, t_cy, t_r, rng)  # realistic irregular shape
        tumor_mask &= ~brainstem_mask  # safety: remove any discretization edge pixels
