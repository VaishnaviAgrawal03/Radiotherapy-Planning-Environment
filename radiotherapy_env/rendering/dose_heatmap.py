...
def render_heatmap(
    ...
    # Tumor fill (semi-transparent green) — makes small tumors visible
    tumor_rgba = np.zeros((*dose.shape, 4), dtype=np.float32)
    tumor_rgba[patient.tumor_mask] = [0.0, 1.0, 0.53, 0.30]  # #00ff88 @ 30% opacity
    ax.imshow(tumor_rgba, origin="upper", zorder=4, interpolation="nearest")

    # Tumor contour (bright green)
    tumor_contour = _get_contour(patient.tumor_mask.astype(np.uint8))
    for contour in tumor_contour:
        ax.plot(contour[:, 1], contour[:, 0], color="#00ff88", linewidth=2.5,
                label="Tumor (PTV)", zorder=6)
    ...
    for beam in beams:
        ...
        x1, y1, x2, y2 = _beam_endpoints(iso_x, iso_y, angle_rad,
                                          patient.body_mask, G)
        ax.plot([x1, x2], [y1, y2], color="white",
                alpha=0.3 + 0.5 * beam.dose_weight, lw=1.2, zorder=3)
...
def _beam_endpoints(iso_x, iso_y, angle_rad, body_mask, G):
    """
    Ray-march from isocenter in both beam directions and stop at the body
    boundary, so beam lines are always clipped to the patient outline.
    Returns (x1, y1, x2, y2) in plot (column, row) coordinates.
    """
    ...
    def march(sign):
        for r in range(1, G):
            xi = int(round(iso_x + sign * r * cos_a))
            yi = int(round(iso_y + sign * r * sin_a))
            if not (0 <= xi < G and 0 <= yi < G and body_mask[yi, xi]):
                return max(0, r - 1)
        return G - 1
    ...
    r_bwd = march(-1)
    return (
        iso_x - r_bwd * cos_a, iso_y - r_bwd * sin_a,
        iso_x + r_fwd * cos_a, iso_y + r_fwd * sin_a,
    )
